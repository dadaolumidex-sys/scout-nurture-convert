...
import { Send, Bot, Plus, ImagePlus, Sparkles, X, Pencil, Trash2, Copy, MoreVertical, Check, RotateCcw, History } from "lucide-react";
...
const ChatPage = () => {
  ...
  return (
    <DashboardLayout>
      <div className={`flex animate-slide-in ${isMobile ? "h-[calc(100vh-7.5rem)]" : "h-[calc(100vh-8rem)]"}`}>
        {/* Desktop sidebar */}
        {!isMobile && <div className="hidden md:flex">{historyPanel}</div>}

        {/* Main chat area */}
        <div className={`flex-1 flex flex-col min-w-0 ${!isMobile ? "pl-4" : ""}`}>
          {/* Header */}
          ...
          <div className={`flex items-center gap-2 ${isMobile ? "mb-2" : "mb-3"}`}>
            ...
            {deepResearch && !isMobile && <Badge variant="outline" className="text-xs border-primary/30 text-primary">Advanced analysis</Badge>}
          </div>
            ...
            {messages.length === 0 && (
              <div className="flex items-center justify-center h-full">
                <div className="text-center text-muted-foreground px-4">
                  ...
                  <p className={isMobile ? "text-xs" : "text-sm"}>Ask anything or upload a conversation for the perfect reply</p>
                </div>
              </div>
            )}
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`group relative rounded-xl px-3 py-2 text-sm ${isMobile ? "max-w-[90%]" : "max-w-[80%]"} ${msg.role === "user" ? "bg-muted text-foreground" : "bg-accent text-accent-foreground border border-border"}`}>
